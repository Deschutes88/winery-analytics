              ...
              Try {
                ...
                val rows: List[String] =
                  for {vintage_id: String <- vintage_ids}
                    yield {
                        val desc = vintages.get(vintage_id)
...
//                        val price = Try{desc.get("price").asText}.getOrElse("")
                        ...
                        s"$wineId\t$vintage_id\t$currency\t$currency_name\t$median\t$typ\t$amount\t$discounted_from\t$price__type\n"
                    }
                log.info(s"${rows.length} rows created from `$p`")
                if(rows.isEmpty) zeroRowsCounter.incrementAndGet()
                if(rows.length != vintage_ids.length)
                  log.error(s"Number of rows ${rows.length} != number of ids ${vintage_ids.length} fpr `$p`!")
                rows.mkString
              }.recoverWith { case e =>
...
//    val headerRow = "id\tcurrency\tcurrency_name\tmedian.amount\tmedian.type\tprice.amount\tprice.discounted_from\tprice.type\n"
    val headerRow = "wine_id\tvintage_id\tcurrency\tcurrency_name\tmedian__amount\tmedian__type\tprice__amount\tprice__discounted_from\tprice__type\n"
